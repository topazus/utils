...
// constant for the filesystem type
const (
...
// map from the file system type constant number to its corresponding string name
...
const (
	...
	// (10) super options: per super block options. "rw"
	SuperOptions = 10
)

// Mount contains all metadata for a single filesystem mount.
type MountInfo struct {
	// (9) mount source: filesystem specific information or "none". "/dev/nvme0n1p1"
	MountSource string `json:"mount_source"`
	// mount point: mount point relative to the process's root. "/"
	MountPoint string `json:"mount_point"`
	// filesystem type: name of filesystem of the form. "/dev/nvme0n1p1"
	Fstype string `json:"fs_type"`
	// partition type, btrfs, ext4, etc.
